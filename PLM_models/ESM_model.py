...
# Print or use best_sequences as needed
 ...
 # Get the list of best sequences
    ...
    def delta_likelihood(self, sequences: list, starts, ends, num_mutations):
        likelihoods = self.process_sequences(sequences, starts, ends)

        mutated_likelihoods = self.mutate_sequences(num_mutations)

        mutated_starts = [0] * len(mutated_likelihoods)
        mutated_ends = [len(seq) for seq in self.mutated_sequences]
        mutated_pseudo_likelihoods = self.calc_pseudo_likelihood_sequence(self.mutated_sequences, mutated_starts, mutated_ends)

        df_delta_likelihood = pd.DataFrame(columns=['Original_sequence', 'Mutated_sequence', 'Delta_score'])

        # Calculate delta score for each pair of original and mutated sequence
        for original_seq, mutated_seq, orig_likelihood, mutated_likelihood in zip(sequences, self.mutated_sequences, likelihoods['Evo_likelihood_original'], mutated_pseudo_likelihoods):
            delta_score = orig_likelihood - mutated_likelihood
            df_delta_likelihood = pd.concat([df_delta_likelihood, pd.DataFrame({'Original_sequence': [original_seq], 'Mutated_sequence': [mutated_seq], 'Delta_score': [delta_score]})], ignore_index=True)


        print(df_delta_likelihood)
        return df_delta_likelihood
